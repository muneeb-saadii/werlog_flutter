...
class _NewCameraScreenState extends State<NewCameraScreen>
  ...
  /// Resolved view config — falls back to defaults when widget.data is null.
  CameraViewData get _data => widget.data ?? CameraViewData();
...
// for camera-captured images — shows a confirm/adjust UI before adding.
  ...
  Future<void> _scanWithCamera() async {
    ...
    if (!_data.allowMultipleImages && _images.isNotEmpty) {
      _showConflictSnackbar('Remove the current image first to replace it.');
      return;
    }
    try {
      final List<String>? pictures =
      await CunningDocumentScanner.getPictures(
        noOfPages: _data.allowMultipleImages ? 10 : 1,
        isGalleryImportAllowed: false,
      );
      if (pictures == null || pictures.isEmpty) return;
      if (mounted) {
        setState(() {
          if (!_data.allowMultipleImages) _images.clear();
  ...
  Future<void> _pickPdf() async {
    if (!_data.allowPdf) return; // defensive — tile is hidden in this mode
...
class _SelectionBody extends StatelessWidget {
  ...
  /// In single-image mode, once one image exists the slot is full —
  /// Camera/Gallery tiles lock until it's removed. Multi-image mode
  /// always has an open slot.
  bool get _slotAvailable => data.allowMultipleImages || images.isEmpty;
          ...
          child: SingleChildScrollView(
            ...
            child: Column(children: [
              ...
              _OptionTile(
                ...
                enabled: selectedPdf == null && _slotAvailable,
                lockedReason: selectedPdf != null
                    ? 'Remove PDF first'
                    : (!_slotAvailable ? 'Remove the image first' : null),
                onTap: onCamera,
                isCamera: true,
              ),
              ...
              _OptionTile(
                ...
                subtitle: data.allowMultipleImages
...
class CameraViewData {
  ...
  /// When false, the user can only capture/pick ONE image total.
  /// Gallery switches to a single-image picker, the document scanner is
  /// capped at 1 page, and once an image exists the Camera/Gallery tiles
  /// are locked until it's removed. Defaults to true (existing behavior).
  final bool allowMultipleImages;

  /// When false, the "Upload PDF" option is hidden entirely and PDF
  /// selection is blocked. Defaults to true (existing behavior).
  final bool allowPdf;
